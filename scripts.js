...
var Game = {
	...
	isGameOver() {
		...
		if(field[0][0] !== '' && field[0][2] === field[1][1] && field[1][1] === field[2][0]) {
			return field[0][2]
		}
		if(this.round === 9) {
			return 'DRAW'
	...
	render() {
		...
		divEl.textContent = winner ? `Winner: ${winner}` : `Current Player: ${this.currentPlayer}`
		...
		this.field.forEach((line, lineIndex) => {
			...
			line.forEach((column, columnIndex) => {
				template += `<td onclick="Game.setPlay(${lineIndex}, ${columnIndex})">${column}</td>`
			})
			template += '</tr>'
		})
